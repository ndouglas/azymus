...
/// Creates a room.
fn create_room(seed: SeedType, _rng: &mut RngType, level: i32, room: Rectangle, map: &mut MapType) {
    for x in (room.x + 1)..room.x2() {
        for y in (room.y + 1)..room.y2() {
            if x % 5 != 0 || y % 5 != 0 {
                map[x][y] = Tile::floor(seed, x as i32, y as i32, level);
            }
        }
    }
}
    ...
    for _ in 0..width / 2 {
        ...
        if !failed {
            ...
            if rooms.is_empty() {
                ...
                if rng.gen() {
                    create_h_tunnel(seed, level, prev_x as i32, new_x as i32, prev_y as i32, &mut map);
                    create_v_tunnel(seed, level, prev_y as i32, new_y as i32, new_x as i32, &mut map);
                } else {
                    create_v_tunnel(seed, level, prev_y as i32, new_y as i32, prev_x as i32, &mut map);
                    create_h_tunnel(seed, level, prev_x as i32, new_x as i32, new_y as i32, &mut map);
                }
            }
            rooms.push(new_room);
        }
